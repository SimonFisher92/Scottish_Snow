...
def snowpatch_bbox_geojson(coords_dict: dict, offset: float):
    '''
    ...
    for name, centroid in coords_dict.items():
        ...
        with open(f'{name}_bbox.geojson', 'w') as f:
    ...
    with open('all_patches_bbox.geojson', 'w') as f:
        json.dump(all_geojson, f, indent=4)


def snowpatch_point_geojson(coords_dict: dict):
    '''
    :param coords_dict: input patch names and coordinates
    :param offset: the offset from the centre of the snowpatch required to the make the bounding box ROI (larger is bigger)
    :return: geojsons for specific patches
    ...
    for name, centroid in coords_dict.items():
        ...
        # Save individual GeoJSON to a file
        with open(f'{name}_point.geojson', 'w') as f:
            json.dump(geojson_data, f, indent=4)
    ...
    with open('all_patches_point.geojson', 'w') as f:
        json.dump(all_geojson, f, indent=4)
...
if __name__ == "__main__":
    ...
    snowpatch_bbox_geojson(test_cords, offset=1e-2)
    snowpatch_point_geojson(test_cords)
